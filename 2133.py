...
class Solution:
    ...
    def checkValid(self, matrix: List[List[int]]) -> bool:
        ...
        for row in matrix:
            ...
            for cell in row:
                isCached = rowCache.get(cell, False)
                if (isCached):
                    return False
                
                rowCache[cell] = True
                
                isColumnCached = self.columnCache.get(columnCount, {}).get(cell, False)
                if (isColumnCached):
                    return False
                
                column = self.columnCache.get(columnCount, {})
                column[cell] = True
                self.columnCache[columnCount] = column
                columnCount += 1
            rowCount += 1
